...
class TicketAPIController extends Controller
{
    ...
    {
        $last_processing_time = Ticket::where('status', '=', '1')->orderBy('id', 'desc')->first();
        if(!is_null($last_processing_time)){
            $last_processing_time = $last_processing_time->updated_at->diffForHumans();
        } else {
            $last_processing_time = "No tickets have been processed.";
        }
